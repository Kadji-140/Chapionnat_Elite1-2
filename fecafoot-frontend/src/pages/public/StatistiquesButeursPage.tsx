// src/pages/public/StatistiquesButeursPage.tsx
...
import { getTopButeurs } from '../../api/statistiques.api';
...
export const StatistiquesButeursPage: React.FC = () => {
  ...
  // Load Buteurs when Competition or filters change
  useEffect(() => {
    if (!selectedCompetition) {
      setStats([]);
      return;
    }
    const loadButeurs = async () => {
      setIsLoading(true);
      try {
        const filters: any = {};
        ...
        const res = await getTopButeurs(Number(selectedCompetition), filters);
        if (res.success && res.data) {
          setStats(res.data);
        }
      } catch (err) {
        console.error(err);
        toast.error(isEn ? 'Error loading scorers.' : 'Erreur de chargement des buteurs.');
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          <h1 className="page-title" style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            {isEn ? 'Top Scorers Standings' : 'Classement des Meilleurs Buteurs'}
          </h1>
          <p className="page-subtitle">
            {isEn 
              ? 'View the standings of players with the most goals scored on the pitch or from penalties.'
              : 'Visualisez le classement des joueurs ayant inscrit le plus de buts sur le terrain ou sur pénalty.'}
          </p>
        </div>
      </div>
...
export default StatistiquesButeursPage;
